...
def process_api_key(api_key_type, api_key, api_secret=None, tenant_id=None, quiet=False):
    try:
        ...
        if api_key_type == "mapbox api key":
            run_script(['/bin/bash', script_name_1, api_key_type, api_key], quiet)
            ...
            if not (api_key and api_secret and tenant_id):
                ...
                return
            run_script(['bin/bash', script_name_3, api_key, api_secret, tenant_id], quiet)
        elif api_key_type in ["microsoft teams webhook", "slack webhook"]:
            run_script(['/bin/bash', script_name_1, api_key_type, api_key], quiet)
        else:
            if api_secret:
                run_script(['/bin/bash', script_name_2, api_key_type, api_key, api_secret], quiet)
            else:
                run_script(['/bin/bash', script_name_1, api_key_type, api_key], quiet)
    except Exception as e:
        if not quiet:
